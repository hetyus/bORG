...
#define COL_SETTLE_US 260
#define MIN_DT_TICKS 300
#define SAMPLES 3
#define MAJ 2
#ifndef INTER_SAMPLE_DELAY_US
  #define INTER_SAMPLE_DELAY_US 30
#endif
#ifndef DROP_FIRST_SAMPLE_AFTER_COL
  #define DROP_FIRST_SAMPLE_AFTER_COL 1
#endif
...
static volatile bool g_discard_first_sample = false;
...
#if DROP_FIRST_SAMPLE_AFTER_COL
  g_discard_first_sample = true;
#endif
}
static inline bool rd(uint8_t pin){
#if DROP_FIRST_SAMPLE_AFTER_COL
  if(g_discard_first_sample){ (void)digitalRead(pin); g_discard_first_sample=false; if(INTER_SAMPLE_DELAY_US) delayMicroseconds(INTER_SAMPLE_DELAY_US); }
#endif
  uint8_t hit=0;
  for(uint8_t i=0;i<SAMPLES;i++){
    if(digitalRead(pin)==LOW) hit++;
    if(INTER_SAMPLE_DELAY_US) delayMicroseconds(INTER_SAMPLE_DELAY_US);
  }
  return hit>=MAJ;
}
/*** MIDI ***/
...
static inline bool shortFocusWait(uint8_t panel,uint8_t col,uint8_t row,bool wantKS,uint16_t &tOutTicks){
  setEnableForPanel(panel); selCol(col); delayMicroseconds(40);
  uint32_t us=0;
